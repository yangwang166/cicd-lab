...
```
dbx execute --cluster-name=<name of interactive cluster> --job=cicd-lab-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx launch --job=cicd-lab-sample-integration-test
```
...
```
dbx launch --job=cicd-lab-sample
```
